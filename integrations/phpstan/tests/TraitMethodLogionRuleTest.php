...
use DoctrineOfTheSecondSun\PHPStan\Rules\TraitMethodLogionRule;
use PHPStan\Rules\Rule;
use PHPStan\Testing\RuleTestCase;

/** @extends RuleTestCase<TraitMethodLogionRule> */
final class TraitMethodLogionRuleTest extends RuleTestCase
{
    protected function getRule(): Rule
    {
        $parser = new LogionParser(['OSD', 'RAS', 'AWC', 'SFA']);

        return new TraitMethodLogionRule(
            $parser,
    ...
    public function testReportsOneErrorWhenTwoClassesUseTheTrait(): void
    {
        $this->analyse([__DIR__ . '/data/trait-validation.php'], [
            [
                'Named declaration ValidatedTrait::missingTraitMethod() must contain exactly one @logion PHPDoc tag.',
                10,
            ],
        ]);
    }
